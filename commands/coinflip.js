...
module.exports.run = async (bot, message, args) => {
    ...
    {
        if(args == "" || isNaN(args[1]) || args[0] == "" || args[1] < 0)
        ...
        return message.channel.send("You don't have enough balance!")
            ...
            UserJSON[message.author.id].bal = UserJSON[message.author.id].bal - args[1]
            fs.writeFileSync("./bani.json", JSON.stringify(UserJSON))
            ...
            if(chance == "heads" && args[0] == "heads") {
                ...
                .setColor("#00ff00")
                .setDescription(`The coin landed on heads and you just won ${args[1] * 2} 💸!`)
                message.channel.send(embed);
                UserJSON[message.author.id].bal = UserJSON[message.author.id].bal + args[1] * 2
                fs.writeFileSync("./bani.json", JSON.stringify(UserJSON))  
            } else if(chance == "tails" && args[0] == "tails") {
                const embed = new Discord.MessageEmbed()
                .setColor("#00ff00")
                .setDescription(`The coin landed on tails and you just won ${args[1] * 2} 💸!`)
                message.channel.send(embed);
                UserJSON[message.author.id].bal = UserJSON[message.author.id].bal + args[1] * 2
                fs.writeFileSync("./bani.json", JSON.stringify(UserJSON)) 
            }
            else
            {
                const embed = new Discord.MessageEmbed()
                .setColor("#ff0000")
                .setDescription(`The coin landed on ${chance}, you lost ${args[1]} 💸, try again!`)
                message.channel.send(embed);
            }
        }
    }
