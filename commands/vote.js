...
module.exports.run = async (bot, message, args) => {
    ...
    .setFooter(`Poll created by ${message.author.username}`, message.author.displayAvatarURL())
    .setTimestamp()
    ...
    collector_age.on('collect', async (reaction, user) => {
        ...
        collector_age.on('end', collected => {
            ...
            .setTitle(`Poll results! (${args[0]} or ${args[1]})`)
            .setDescription(`${collected.get('1️⃣').count-1} people voted for ${args[0]} and ${collected.get('2️⃣').count-1} people voted for ${args[1]}`)
            .setColor("#94CD62")
